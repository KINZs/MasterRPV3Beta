...
{
	name = "Custom guns",
	author = "Alienmario Extended by Master(D)",
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, err_max)
{
	...
	CreateNative("CG_CanClientReload", Native_CanClientReload);
	CreateNative("CG_SetCanClientReload", Native_SetCanClientReload);
	CreateNative("CG_HasInInventory", Native_HasInInventory);
...
public Native_CanClientReload(Handle plugin, numParams)
{
	int client = GetNativeCell(1);
	
	if(canreload[client] == true)
	{
...
public Native_SetCanClientReload(Handle plugin, numParams)
{
	int client = GetNativeCell(1);
	int Selected = GetNativeCell(2);

	if(Selected == 1)
	{
	
		canreload[client] = true;
		return;
	}
	
	canreload[client] = false;
}

public Native_HasInInventory(Handle plugin, numParams)
{
	int Client = GetNativeCell(1);
	char classname[32];
	GetNativeString(2, classname, sizeof(classname));
	
	//PrintToServer("%N has tried to equipt %s but has %i", Client, classname, hasInInventory(Client, getIndex(classname)));
	return hasInInventory(Client, getIndex(classname));
}
...
public OnPluginStart()
{
	...
	customguns_default = CreateConVar("customguns_default", "weapon_fists", "The preferred custom weapon that players should spawn with");
	...
	ReloadForward = CreateGlobalForward("CG_OnReload", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	WeaponEquiptForward = CreateGlobalForward("CG_OnWeaponEquipt", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	AddToInventoryForward = CreateGlobalForward("CG_OnAddToInventory", ET_Ignore, Param_Cell, Param_String);
	CustomGunsMenuFunctionAndReloadForward = CreateGlobalForward("CG_OnPlayerRunCmd", ET_Ignore, Param_Cell, Param_Cell);
	...
	if (GetArraySize(gunClassNames) > 0) {
		...
		if (ent != -1) {
			...
			if (switchTo)
			{
				...
				Call_StartForward(WeaponEquiptForward);
				Call_PushCell(client);
				Call_PushCell(ent);
				Call_PushCell(switchTo);
				Call_Finish();
			}
		} else {
			selectedGunIndex[client] = -1;
...
public Action OnPlayerRunCmd(client, &buttons, &impulse, float vel[3], float angles[3], &weapon, &subtype, &cmdnum, &tickcount, &seed, mouse[2])
{
	if(!IsFakeClient(client)){
		...
		if (!open[client] && IsPlayerAlive(client) && !zooming(client) && inventory[client] && GetArraySize(inventory[client])>0) {
...
/*
	Call_StartForward(CustomGunsMenuFunctionAndReloadForward);
	Call_PushCell(client);
