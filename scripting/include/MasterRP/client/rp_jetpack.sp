...
public void initJetPack()
{
	...
	//Commands:
	RegConsoleCmd("+jetpack", JetpackP, "", FCVAR_GAMEDLL);

	RegConsoleCmd("-jetpack", JetpackM, "", FCVAR_GAMEDLL);
}
...
public void StartJetPack(int Client)
{
	...
	//Jetpack:
	else if(JetPackEffect[Client] == 10)
	{

		//Create Effect:
		JetPackEnt[Client] = CreateCombineBallJetpack(Client);
	}
}
...
public void StopJetPack(int Client)
{
	...
	if(IsValidEdict(JetPackEnt[Client]))
	{
		...
		if(IsValidEntity(Effect))
		{
			...
			//Request:
			RequestFrame(OnNextFrameKill, Effect);
		...
		//Request:
		RequestFrame(OnNextFrameKill, JetPackEnt[Client]);
...
public bool IsJetpackOn(int Client)
{
	...
	//Return:
	return view_as<bool>(JetPack[Client]);
}
...
public bool IsJetpackEnabled(int Client)
{
	...
	//Return:
	return view_as<bool>(JetPackEnabled[Client]);
}
...
public int GetJetPackEffect(int Client)
{
	...
	//Return:
	return view_as<int>(JetPackEffect[Client]);
}
...
public int GetJetPackEnt(int Client)
{

	//Return:
	return view_as<int>(JetPackEnt[Client]);
}

public void SetJetPackEnt(int Client, int Ent)
{

	//Initulize:
	JetPackEnt[Client] = Ent;
}

public int CreateRofleChopter(int Client)
{
	...
	//Create Effect:
	//int Ent = CreateProp(Position, Angles, "models/props_c17/trappropeller_blade.mdl", true, false);
	int Ent = CreateDynamicProp(Position, Angles, "models/props_c17/trappropeller_blade.mdl", true);


	//Set Prop ClassName

	SetEntPropString(Ent, Prop_Data, "m_iClassname", "RofleChopter");
	...
	int Ent2 = CreateDynamicProp(Position, Angles, "models/props_c17/trappropeller_blade.mdl", true);


	//Set Prop ClassName

	SetEntPropString(Ent2, Prop_Data, "m_iClassname", "RofleChopter");


	//Declare:
	Collision = GetEntSendPropOffs(Ent2, "m_CollisionGroup");
	...
	//Accept:
	AcceptEntityInput(Ent2, "SetParent", Client, Ent2, 0);

	//Attach:
	SetVariantString("eyes");

	//Accept:
	AcceptEntityInput(Ent2, "SetParentAttachment", Ent2, Ent2, 0);
	...
	//Remove Index:
	SetEntAttatchedEffect(Ent2, 1, Ent);

	//SDKHOOK:
	SDKHook(Ent2, SDKHook_SetTransmit, OnJetPackSetTransmit);

	//Return:
	return view_as<int>(Ent2);
}

public int CreateCombineBallJetpack(int Client)
{
	...
	//Create Effect:
	//int Ent = CreateProp(Position, Angles, "models/props_c17/trappropeller_blade.mdl", true, false);
	int Ent = CreateDynamicProp(Position, Angles, "models/effects/combineball.mdl", true);
	...
	SetEntPropFloat(Ent, Prop_Send, "m_flModelScale", 3.5);
	...
	//Initulize Effects:
	int Effect = CreateLight(Ent, 1, 150, 160, 170, "null");
...
public int CreateJetPack(int Client)
{
	...
	//Create Effect:
	//int Ent = CreateProp(Position, Angles, "models/ice_dragon/jetpack/blue_jetpack.mdl", true, false);
	int Ent = CreateDynamicProp(Position, Angles, "models/ice_dragon/jetpack/blue_jetpack.mdl", true);

	//Set Prop ClassName

	SetEntPropString(Ent, Prop_Data, "m_iClassname", "JetPack");
		...
		if(Client > 0 && IsClientInGame(Client) && IsClientConnected(Client))
		{
			...
			if(Result == 0)
			{
				...
				if(JetPackEnabled[Client] == false)
				{
					...
					//Print:
					CPrintToChat(Client, "%s JetPack has been Activated.", PREFIX);
				}
				...
				else if(JetPackEnabled[Client] == true)
				{
					...
					//Print:
					CPrintToChat(Client, "%s JetPack has been Deactivated.", PREFIX);
				}
...
public void JetPackEffectMenu(int Client)
{
	...
	else if(HasItemTypeInInventory(Client, 60))
	{
		...
		//HasItem:
		if(GetItemAmount(Client, 452))
		{

			//Menu Button:
			menu.AddItem("10", "Combine Ball JetPack!");
		}
...
public int HandleJetPackEffectMenu(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		if(Client > 0 && IsClientInGame(Client) && IsClientConnected(Client))
		{
			...
			//Print:
			CPrintToChat(Client, "%s Your new JetPack Effect is a %s%s!", PREFIX, COLORGREEN, display);
