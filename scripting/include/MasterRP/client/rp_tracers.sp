...
public void OnClientShowTracers(int Client)
{
	...
	if((IsCop(Client) || IsAdmin(Client)))
	{
		...
		for(int i = 1; i <= GetMaxClients(); i ++)
		{
			...
			if(IsClientConnected(i) && IsClientInGame(i) && Client != i)
			{
				...
				//Is Cop + Beam Can Use:
				if(IsPlayerAlive(i) && GetCrime(i) > MINCRIMETRACE && GetCrimeTracer(Client) == 1 && GetPoliceJammerTime(i) < GetGameTime() && GetBountyJammerTime(i) < GetGameTime())
				{
					...
					//Initulize:
					GetEntPropVector(i, Prop_Data, "m_vecOrigin", CriminalOrigin);
	...
	//Check:
	if(GetBountyTracer(Client) == 1 && !IsCop(Client) && !IsAdmin(Client))
	{
	...
	//Check:
	if(GetPoliceScannerTime(Client) > GetGameTime())
	{
		...
		for(int i = 1; i <= GetMaxClients(); i ++)
		{
			...
			if(IsClientConnected(i) && IsClientInGame(i) && Client != i)
			{
				...
				if(IsPlayerAlive(i) && IsCop(i))
				{
					...
					//Initulize:
					GetEntPropVector(i, Prop_Data, "m_vecOrigin", CopOrigin);
	...
	//Override:
	if(GetGpsBugTime(Client) > GetGameTime())
	{
		...
		for(int i = 1; i <= GetMaxClients(); i ++)
		{
			...
			//Connected:
			if(IsClientConnected(i) && IsClientInGame(i) && GetGpsBug(Client) == i)
			{
				...
				if(IsPlayerAlive(i))
				{
					...
					if(CheckMapEntityCount() < 2047)
					{
						...
						//End Temp:
						TE_SetupBeamPoints(ClientOrigin, Origin, Laser(), 0, 0, 66, 0.25, 1.0, 3.0, 0, 0.0, BeamColor, 0);
			...
			//Disconnected:
			if(!IsClientConnected(i) && GetGpsBug(Client) == i)
			{
				...
				//Print:
				CPrintToChat(Client, "%s You Gps Bug has stopped working.", PREFIX);
			}
		}
	}

	//Time Over:
	if(GetGpsBugTime(Client) < GetGameTime() && GetGpsBug(Client) != -1)
	{
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			if(IsCop(i) || IsAdmin(i))
			{
				...
				if(Dist <= 800)
				{
					...
					TE_SetupBeamRingPoint(Origin, 1.0, 40.0, Laser(), Sprite(), 0, 20, 1.2, 3.0, 0.5, Color, 7, 0);
