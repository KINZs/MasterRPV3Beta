...
public void T_DBLoadLastStats(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		else if(SQL_FetchRow(hndl))
		{
			...
			//Set Max Health so the player can be healed
			...
			//Load:
			LoadLotteryTickets(Client);

			//TestSpinTable:
			DBLoadSpinTest(Client);

			//Print:
			PrintToConsole(Client, "|RP| Player Last Stats Loaded");

			//Is Cop and Has Crime:
			if(IsCop(Client) && GetCrime(Client) > 500)
			{
				...
				//Print:
				CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have been switched because you have crime.");
