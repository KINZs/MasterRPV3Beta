...
int HudInfo[MAXPLAYERS + 1] = {0,...};
int HudOnline[MAXPLAYERS + 1] = {1,...};
...
public Action CreateSQLdbsettings(Handle Timer)
{
	...
	len += Format(query[len], sizeof(query)-len, " `HudInfo` int(12) NOT NULL DEFAULT 0,");
	...
	len += Format(query[len], sizeof(query)-len, " `DoubleJump` int(5) NOT NULL DEFAULT 1,");

	len += Format(query[len], sizeof(query)-len, " `DoubleJumpEffect` int(5) NOT NULL DEFAULT 0,");

	len += Format(query[len], sizeof(query)-len, " `HudOnline` int(5) NOT NULL DEFAULT 1)");

	//Thread Query:
	SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query, 55);

	//Format(query, sizeof(query), "ALTER TABLE `Settings` ADD `DoubleJump` int(5) NOT NULL DEFAULT 1;");

	//Thread Query:
	//SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query);

	//Format(query, sizeof(query), "ALTER TABLE `Settings` ADD `HudOnline` int(5) NOT NULL DEFAULT 1;");
...
public void InsertSettings(int Client)
{
	...
	//Sql String:
	Format(buffer, sizeof(buffer), "INSERT INTO Settings (`STEAMID`,`ClientHud`,`PlayerHud`,`CrimeTracer`,`MoreHud`,`HudEnable`,`HudInfo`,`CallEnable`,`RingEnable`,`DrugPing`,`TrashTracer`,`BountyTracer`) VALUES (%i,'120 120 255', '120 120 255', 1, 1, 1, 0, 1, 1, 0, 0, 1);", SteamIdToInt(Client));
...
public void T_DBLoadSettingsCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		else if(SQL_FetchRow(hndl))
		{
			...
			//Database Field Loading INTEGER:
			SetJumpEnabled(Client, intTobool(SQL_FetchInt(hndl, 16)));

			//Database Field Loading INTEGER:
			SetJumpEffect(Client, intTobool(SQL_FetchInt(hndl, 17)));

			//Database Field Loading INTEGER:
			HudOnline[Client] = SQL_FetchInt(hndl, 18);
...
public void ClientSettingsMenu(int Client)
{
	...
	menu.AddItem("7", "Trails");

	menu.AddItem("8", "Double Jump");
...
public int HandleSettings(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		if(Result == 0)
		{
			...
			//Format:
			Format(State, sizeof(State), "Hud Online is %s", HudOnline[Client] ? "on" : "off");

			//Menu Button:
			menu.AddItem("3", State);

			//Menu Button:
			menu.AddItem("4", "Change Hud color");

			menu.AddItem("5", "Change Player Color");

			menu.AddItem("6", "Change Entity Color");
...
public int HandleHud(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		if(Result == 2)
		{
			...
			//Sql Strings:
			Format(query, sizeof(query), "UPDATE Settings SET HudInfo = %i WHERE STEAMID = %i;", HudInfo[Client], SteamIdToInt(Client));
		...
		if(Result == 3)
		{
			...
			//Print:
			CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Hud Online has been toggled.");

			//Declare:
			char query[255];

			//Sql Strings:
			Format(query, sizeof(query), "UPDATE Settings SET HudOnline = %i WHERE STEAMID = %i;", HudOnline[Client], SteamIdToInt(Client));

			//Not Created Tables:
			SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query, 612);
		}
		...
		if(Result == 4)
		{
			...
			menu.AddItem("0 250 220 50", "Yellow");
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		if(Result == 3)
		{
			...
			if(IsAdmin(Client))
			{
				...
				menu.AddItem("models/perftest/gman.mdl", "hl1 Gman");
...
public void SetTalkZone(int Client)
{
...
public bool GetCallEnable(int Client)
{

	//Return:
	return view_as<bool>(CallEnable[Client]);
}
...
public bool GetRingEnable(int Client)
{
	...
	//Return:
	return view_as<bool>(RingEnable[Client]);
}
...
public int GetMoreHud(int Client)
{
	...
	//Return:
	return view_as<int>(MoreHud[Client]);
}

public int GetHudEnable(int Client)
{

	//Return:
	return view_as<int>(HudEnable[Client]);
}
...
public int GetHudInfo(int Client)
{
	...
	//Return:
	return view_as<int>(HudInfo[Client]);
}

public int GetHudOnline(int Client)
{
...
public int GetPing(int Client)
{
	...
	//Return:
	return view_as<int>(PingOn[Client]);
}
...
public int GetTrashTracer(int Client)
{
	...
	//Return:
	return view_as<int>(TrashTracer[Client]);
}
...
public int GetCrimeTracer(int Client)
{
	...
	//Return:
	return view_as<int>(CrimeTracer[Client]);
}
...
public int GetBountyTracer(int Client)
{
	...
	//Return:
	return view_as<int>(BountyTracer[Client]);
}

public void SetMoreHud(int Client, int Amount)
{
...
public int GetClientHudColor(int Client, int Id)
{
	...
	//Return:
	return view_as<int>(ClientHudColor[Client][Id]);
}
...
public int GetPlayerHudColor(int Client, int Id)
{
	...
	//Return:
	return view_as<int>(PlayerHudColor[Client][Id]);
}
...
public int GetEntityHudColor(int Client, int Id)
{
	...
	//Return:
	return view_as<int>(EntityHudColor[Client][Id]);
}


public void initHudColor(int Client)
{
