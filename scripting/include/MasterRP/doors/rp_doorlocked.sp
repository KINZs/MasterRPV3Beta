...
public void T_DBLoadBuyable(Handle owner, Handle hndl, const char[] error, any Ent)
{
...
public Action Command_SetDoorPrice(int Client, int Args)
{
	...
	if(Price <= 0)
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP-Door|\x07FFFFFF - Invalid Door Price %i, Please select 1 or more or use sm_remdoorprice to remove from database!", Entdoor);
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP-Door|\x07FFFFFF - Door \x0732CD32%d\x07FFFFFF has \x0732CD32%s\x07FFFFFF Price set on it!", Entdoor, IntToMoney(Price));
...
public void T_DBPrintDoorsPrice(Handle owner, Handle hndl, const char[] error, any data)
{
...
public Action DoorHud(int Client, int Ent, float NoticeInterval)
{
	...
	//Not For Sale:
	if(IsBuyable[Ent] == false)
	{

		//Is Main Door for sale:
		if(IsBuyable[GetMainDoorId(Ent)] == true)
		{

			//Format:
			len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Main door is For Sale : %s", IntToMoney(DoorPrice[Ent]));
		}
	}
...
public int GetMainDoorId(int Ent)
{
...
public bool GetDoorLocked(int Ent)
{
...
public void SetDoorLocked(int Ent, bool Result)
{
	...
	else
	{
		...
		if(hDatabase)
		{
			...
			if(fetch)
			{
				...
				//Format:
				Format(query, sizeof(query), "UPDATE DoorLocks SET Locks = %i WHERE Map = '%s' AND DoorId = %i;", Amount, ServerMap(), Ent);
			}
			...
			else
			{
				...
				//Format:
				Format(query, sizeof(query), "INSERT INTO DoorLocks (`Locks`,`Map`,`DoorId`) VALUES (%i,'%s',%i);", Amount, ServerMap(), Ent);
			}
...
public int GetDoorPrice(int Ent)
{
...
public void SetDoorPrice(int Ent, int Amount)
{
	...
	else
	{
		...
		if(hDatabase)
		{
			...
			if(fetch)
			{
				...
				//Format:
				Format(query, sizeof(query), "UPDATE DoorPrice SET Price = %i WHERE Map = '%s' AND DoorId = %i;", Amount, ServerMap(), Ent);
			}
...
public bool IsDoorBuyable(int Ent)
{
...
public void OnItemsUseAddLocks(int Client, int ItemId)
{
	...
	if(IsValidEdict(DoorEnt))
	{
		...
		if(StrEqual(ClassName, "func_door") || StrEqual(ClassName, "func_door_rotating") || StrEqual(ClassName, "prop_door_rotating"))
		{
			...
			//Check:
			if(!NativeIsPublicDoor(DoorEnt))
			{

				//Check:
				if(!NativeIsCopDoor(DoorEnt) && !IsAdmin(Client))
				{
					...
					else
					{
						...
						//Print:
						CPrintToChat(Client,"\x07FF4040|RP|\x07FFFFFF - Adding locks to public doors is not allowed!");
					}
				}
				...
				else
				{
					...
					//Print:
					CPrintToChat(Client,"\x07FF4040|RP|\x07FFFFFF - Adding locks to cop doors is not allowed!");
				}
			}
...
public void OnItemsUseRemLocks(int Client, int ItemId)
{
