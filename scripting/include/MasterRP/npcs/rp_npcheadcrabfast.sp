...
//Event Damage:
public Action OnClientDamageHeadCrabFast(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
	//Check:
...
public int CreateNpcHeadCrabFast(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	//Initialize:
	int NPC = CreateEntityByName("npc_Headcrab_Fast");

	//Is Valid:
	if(NPC > 0)
	{

		DispatchKeyValue(NPC, "ClassName", "npc_HeadcrabFast");
		...
		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnHeadCrabFastDied, true);
