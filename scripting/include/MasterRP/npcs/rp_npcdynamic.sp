...
//Init:
public void SetOnMapStartRelationship(int Entity, char[] ClassName)
{

	//Set Relationship Status To Players:
	SetNPCRelationShipStatus(Entity, false, true, false);

	SetHateAntLionRelationshipStatus(Entity);

	SetHateCombineRelationshipStatus(Entity);

	SetHateZombieRelationshipStatus(Entity);

	SetHateVortigauntRelationshipStatus(Entity);
}
...
public Action NpcHealthHud(int Client, int Ent, float NoticeInterval)
{
	...
	//Format String:
	ReplaceString(ClassName, sizeof(ClassName), "npc_", "");

	//Declare:
	char FormatMessage[255];

	//Format:
	Format(FormatMessage, sizeof(FormatMessage), "Enemy: %s (%i‰)", ClassName, GetEntHealth(Ent));
	...
	if(GetGame() == 2 || GetGame() == 3)
	{
		...
		//Show Hud Text:
		CSGOShowHudTextEx(Client, 1, view_as<float>({-1.0, -0.805}), view_as<int>({220, 220, 50, 255}), view_as<int>({220, 220, 50, 255}), (NoticeInterval + 0.05), 0, 6.0, 0.0, (NoticeInterval), FormatMessage);
	}
	...
	else
	{
		...
		//Show Hud Text:
		ShowHudTextEx(Client, 1, view_as<float>({-1.0, -0.805}), view_as<int>({220, 220, 50, 255}), (NoticeInterval + 0.05), 0, 6.0, 0.0, (NoticeInterval), FormatMessage);
	}
}
...
public void SetPoliceHateRelationshipStatus(int Entity)
{
...
public void SetCitizenLikeRelationshipStatus(int Entity)
{
	...
	for(int i = 1; i <= GetMaxClients(); i++)
	{
		...
		//Format:
		Format(ClientIndex, sizeof(ClientIndex), "Citizen%i D_LI", i);
...
public void SetCitizenHateRelationshipStatus(int Entity)
{
	...
	for(int i = 1; i <= GetMaxClients(); i++)
	{
		...
		//Format:
		Format(ClientIndex, sizeof(ClientIndex), "Citizen%i D_HT", i);
...
public void SetClientClass(int Client)
{
	...
	if(IsCop(Client))
	{
		...
		SetEntityClassName(Client, ClientIndex);
...
public void SetLikeVortigauntRelationshipStatus(int Entity)
{

	//Set Hate Status
	SetVariantString("npc_Vortigaunt D_LI");

	//Accept:
	AcceptEntityInput(Entity, "setrelationship");

	//Set Hate Status
	SetVariantString("npc_VortigauntSlave D_LI");

	//Accept:
	AcceptEntityInput(Entity, "setrelationship");
}

public void SetHateVortigauntRelationshipStatus(int Entity)
{
	...
	//Set Hate Status
	SetVariantString("npc_VortigauntSlave D_HT");
...
public void SetLikeCombineRelationshipStatus(int Entity)
{
	...
	//Set Hate Status
	SetVariantString("npc_clawscanner D_LI");
