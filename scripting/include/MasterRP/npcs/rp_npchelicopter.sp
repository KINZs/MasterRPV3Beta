...
//Event Damage:
public Action OnClientDamageHelicopter(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
...
public int CreateNpcHelicopter(const char[] sNpc, const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnHelicopterDied, true);
