...
#if defined HL2DM
public void initNpcPoisonZombie()
{

	//NPC Beta:
	RegAdminCmd("sm_testpoisonzombie", Command_CreateNpcPoisonZombie, ADMFLAG_ROOT, "<No Arg>");
}

//Event Damage:
public Action OnPoisonZombieDamageClient(int Client, int &attacker, int &inflictor, float &damage, int &damageType)
{
...
//Event Damage:
public Action OnClientDamagePoisonZombie(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
...
//Ant Lion Died Event:
public void OnPoisonZombieDied(const char[] Output, int Caller, int Activator, float Delay)
{
...
//Create NPC:
public Action Command_CreateNpcPoisonZombie(int Client, int Args)
{
...
public int CreateNpcPoisonZombie(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	//Initialize:
	int NPC = CreateEntityByName("npc_PoisonZombie");

	//Is Valid:
	if(NPC > 0)
	{

		DispatchKeyValue(NPC, "ClassName", "npc_PoisonZombie");
		...
		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnPoisonZombieDied, true);
