...
//Event Damage:
public Action OnClientDamageVortigaunt(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
...
//Ant Lion Died Event:
public void OnVortigauntDied(const char[] Output, int Caller, int Activator, float Delay)
{
...
public int CreateNpcVortigaunt(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		DispatchKeyValue(NPC, "ClassName", "npc_Vortigaunt");
		...
		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnVortigauntDied, true);
