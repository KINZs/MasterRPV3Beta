...
#if defined _rp_npczombie_included_
  #endinput
#endif
#define _rp_npczombie_included_
#if defined HL2DM
public void initNpcZombie()
{

	//NPC Beta:
	RegAdminCmd("sm_testzombie", Command_CreateNpcZombie, ADMFLAG_ROOT, "<No Arg>");
}

//Event Damage:
public Action OnZombieDamageClient(int Client, int &attacker, int &inflictor, float &damage, int &damageType)
{
...
//Event Damage:
public Action OnClientDamageZombie(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
...
//Ant Lion Died Event:
public void OnZombieDied(const char[] Output, int Caller, int Activator, float Delay)
{
	...
	RemoveZombieNPC(Caller);
}

//Create NPC:
public Action Command_CreateNpcZombie(int Client, int Args)
{
	...
	CreateNpcZombie("null", Position, Angles, 200);

	//Return:
	return Plugin_Handled;
}

public int CreateNpcZombie(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		DispatchKeyValue(NPC, "ClassName", "npc_Zombie");
		...
		//Damage Hook:
		SDKHook(NPC, SDKHook_OnTakeDamage, OnClientDamageZombie);

		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnZombieDied, true);
