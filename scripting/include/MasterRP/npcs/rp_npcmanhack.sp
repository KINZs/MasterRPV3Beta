...
//Event Damage:
public Action OnClientDamageManHack(int Entity, int &Client, int &inflictor, float &damage, int &damageType)
{
...
public int CreateNpcManHack(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		//Death Hook:
		HookSingleEntityOutput(NPC, "OnDeath", OnManHackDied, true);
