...
public void T_DBLoadBombZones(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		//Print:
		PrintToServer("|RP| - Bomb Zones Found!");
	}
}
...
public void initGlobalBombTick()
{
	...
	//TimerCheck
	if(BombZoneTimer >= GetBombSpawnTimer())
	{
...
public void ExplodeGlobalBomb(int Ent)
{
	...
	//Create Fire Effect!
	int Effect = CreateInfoParticleSystemOther(Ent, "null", "Fire_Large_01", 0.5, Offset, Angles);

	//Accept:
	AcceptEntityInput(Ent, "ClearParent", Effect);

	Effect = CreateInfoParticleSystemOther(Ent, "null", "striderbuster_break_flechette", 0.5, Offset, Angles);
	...
	//Initulize Effects:
	Effect = CreateEnvFire(GlobalBombEnt, "null", "200", "700", "0", "Natural");
...
public void RemoveGlobalBomb(int Ent)
{
	...
	//Request:
	RequestFrame(OnNextFrameKill, GlobalBombEnt);
...
public int SpawnGlobalBomb(int Var)
{
	...
	//Declare:
	int Ent = CreateProp(BombZones[Var], Angles, "models/Items/item_item_crate.mdl", true, false);
	...
	//Set Prop ClassName
	SetEntityClassName(Ent, "prop_Bomb_global");
...
public int GetGlobalBombEnt()
{
	...
	//Return:
	return view_as<int>(GlobalBombEnt);
}
