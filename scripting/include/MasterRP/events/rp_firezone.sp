...
public void T_DBLoadFireZones(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		//Print:
		PrintToServer("|RP| - Fire Zones Found!");
	}
}
...
public void initGlobalFireTick()
{
	...
	//TimerCheck
	if(FireZoneTimer >= GetFireSpawnTimer())
	{
...
public void ExplodeGlobalFire(int Ent)
{
	...
	//Create Effect:
	CreateEnvExplosion(Ent, "null", 1000.0, 500);
...
public void RemoveGlobalFire(int Ent)
{
	...
	//Request:
	RequestFrame(OnNextFrameKill, GlobalFireEnt);
...
public int SpawnGlobalFire(int Var)
{
	...
	//Declare:
	int Ent = CreateProp(FireZones[Var], Angles, "models/Items/item_item_crate.mdl", true, true);
...
public int GetGlobalFireEnt()
{
	...
	//Return:
	return view_as<int>(GlobalFireEnt);
}
