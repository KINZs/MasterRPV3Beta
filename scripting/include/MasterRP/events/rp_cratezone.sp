...
public void initCrateTick()
{
	...
	//TimerCheck
	if(SupplyCrateTimer >= GetCrateSpawnTimer())
	{
...
public Action OnCrateUse(int Client, int Ent)
{
	...
	if(IsInDistance(Client, Ent))
	{
		...
		if(Random >= 30 && Random < 40)
		{
			...
			//Save:
			SaveItem(Client, 224, (GetItemAmount(Client, 224) + R));

			//Print
			CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You Found %ix of %s!", R, GetItemName(224));
		}
		...
		//Request:
		RequestFrame(OnNextFrameKill, CrateEnt);
...
public int SpawnCrate(int Var)
{
	...
	//Declare:
	int Ent = CreateProp(CrateZones[Var], Angles, "models/Items/item_item_crate.mdl", true, false);
