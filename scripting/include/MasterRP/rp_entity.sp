...
public int CreatePropDoorRotating(char[] Model, char[] Skin, char[] Damage, float Position[3], float Angles[3])
{
	...
	if(IsValidEdict(Door))
	{
		...
		DispatchKeyValue(Door, "skin", Skin);
		...
		DispatchKeyValue(Door, "opendir", "0");

		DispatchKeyValue(Door, "spawnflags", "8192");

		DispatchKeyValue(Door, "OnFullyOpen", "!caller,close,,3,-1");

		DispatchKeyValue(Door, "dmg", Damage);

		//Spawn
		DispatchSpawn(Door);

		//Teleport:
		TeleportEntity(Door, Position, Angles, NULL_VECTOR);
...
public void EntityDissolve(int Ent, int Type)
{
	...
	if(Dissolver > 0 && IsValidEdict(Ent))
	{
		...
		//Request
		RequestFrame(OnNextFrameKill, Dissolver);
	}
}
...
public int CreateInfoParticleSystemOther(int Ent, char[] Attachment, char[] Model, float Time, float Offset[3], float Angles[3])
{
	...
	if(IsValidEdict(Particle) && IsValidEdict(Ent))
	{
		...
		if(Time >= 0.00)
		{
			...
			//Timer:
			CreateTimer(Time, RemoveInfoParticleSystem, Particle);
		}
...
public int CreateSporeExplosion(int Ent, char[] Attachment)
{

	//Declare:
	int Particle = CreateEntityByName("env_sporeexplosion");
...
public Action RemoveInfoParticleSystem(Handle Timer, any Ent)
{
...
public int CreateEnvBloodOther(char[] Attachment, float Position[3], float Direction[3], float Time)
{
...
public int CreateProp(float Origin[3], float Angles[3], char[] Model, bool WalkThru, bool Stuck)
{
...
public int CreateDynamicProp(float Origin[3], float Angles[3], char[] Model, bool WalkThru)
{
...
//CreateCustomLight(Client, 1, 120, 120, 255, 0.5, "120", "80", "4", "0", "null")
...
public int CreateEnvSpriteOrientaded(int Ent, char[] Attachment, char[] Model, char[] Scale, float Offset[3], int R, int G, int B)
{

	//Declare:
	int SpriteOrientaded = CreateEntityByName("env_sprite_oriented");
...
//model, "materials/sprites/bomb_planted_ring.vmt", rendermode, "3, renderfx", "14", scale, "0.2", renderamt, "255", rendercolor, "0 255 0 255");

public int CreateGlow(int Ent, const  char[] Attachment, const char[] Model, const char[] RenderMode2, const char[] RenderFX, const char[] Scale, const char[] RenderAmt, const char[] RenderColor)
{

	//Declare:
	int EnvGlow = CreateEntityByName("env_glow");

	//Check:
	if(IsValidEdict(EnvGlow) && IsValidEdict(EnvGlow))
	{

		DispatchKeyValue(EnvGlow, "model", Model);

		
		DispatchKeyValue(EnvGlow, "rendermode", RenderMode2);


		DispatchKeyValue(EnvGlow, "renderfx", RenderFX);


		DispatchKeyValue(EnvGlow, "scale", Scale);


		DispatchKeyValue(EnvGlow, "renderamt", RenderAmt);


		DispatchKeyValue(EnvGlow, "rendercolor", RenderColor);


		//Set Owner
		SetEntPropEnt(EnvGlow, Prop_Send, "m_hOwnerEntity", Ent);

		//Spawn
		DispatchSpawn(EnvGlow);

		//Declare:
		float Position[3];

		//Initulize:
		GetEntPropVector(EnvGlow, Prop_Send, "m_vecOrigin", Position);

		//Teleport:
		TeleportEntity(EnvGlow, Position, NULL_VECTOR, NULL_VECTOR);

		//Set String:
		SetVariantString("!activator");

		//Accept:
		AcceptEntityInput(EnvGlow, "SetParent", Ent, EnvGlow, 0);
		...
		if(!StrEqual(Attachment, "null"))
		{
			...
			//Accept:
			AcceptEntityInput(EnvGlow, "SetParentAttachment", EnvGlow, EnvGlow, 0);
		}

		//Activate:
		ActivateEntity(EnvGlow);

		//Accept:
		AcceptEntityInput(EnvGlow, "ShowSprite");
...
public int CreateEnvheadcrabcanister(float Position[3])
{

	//Canister:
	int Canister = CreateEntityByName("env_HeadCrabCanister");
	...
	if(IsValidEdict(Canister))
	{
		...
		//Properties:
		DispatchKeyValue(Canister, "HeadcrabType", "0");

		DispatchKeyValue(Canister, "HeadcrabCount", "0");

		DispatchKeyValue(Canister, "FlightSpeed", "5");

		DispatchKeyValue(Canister, "FlightTime", "15");

		DispatchKeyValue(Canister, "StartingHeight", "35");

		DispatchKeyValue(Canister, "SkyboxcanisterCount", "1");

		DispatchKeyValue(Canister, "Damage", "100");

		DispatchKeyValue(Canister, "DamageRadius", "25");

		DispatchKeyValue(Canister, "SmokeLifetime", "1");

		//Spawn:
		DispatchSpawn(Canister);

		//Teleport:
		TeleportEntity(Canister, Position, NULL_VECTOR, NULL_VECTOR);

		//Accept:
		AcceptEntityInput(Canister, "FireCanister");

		//Timer:
		CreateTimer(45.0, RemoveBlood, Canister);
...
public Action CreateExplosion(int Ent, int Ent2)
{
	...
	//CreateDamage:
	ExplosionDamage(Ent, Ent, Origin, DMG_BURN);
...
public void OnNextFrameKill(any data) 
{
	...
	if(data == 0)
	{
		...
		//Print:
		PrintToServer("|RP| - Invalid Entity. server entity selected for killing.");
	}

	//Check:
	else if(data > 0 && IsValidEdict(data))
	{
