					...
					for(int X = 1; X < MAXITEMSPAWN; X++)
					{
						...
						if(PlantEnt[i][X] == Ent)
						{
							...
							if(IsPlanted[i][X] == 1)
							{
								...
								//Is Valid:
								else if(PlantTime[i][X] > 1 && Client != i)
								{
									...
									if(PlantTime[i][X] > 500)
									{
										...
										CPrintToChat(Client, "\x07FF4040|RP-Plant|\x07FFFFFF - You have stolen this plant but it is to premature to harvest properly!", PlantGrams[i][X]);
								...
								else	
								{
									...
									//Print:
									CPrintToChat(Client, "\x07FF4040|RP-Plant|\x07FFFFFF - Plant not ready to harvest. (\x0732CD32%i\x07FFFFFF) Seconds left!", PlantTime[i][X]);
								}
							}
							...
							else	
							{
								...
								if(IsCop(Client))
								{
									...
									//Print:
									CPrintToChat(Client, "\x07FF4040|RP-Plant|\x07FFFFFF - There nothing illegal going on here!");
								}
								...
								else	
								{
									...
									//Print:
									CPrintToChat(Client, "\x07FF4040|RP-Plant|\x07FFFFFF - There are no seeds in this pot to grow!");
								}
							}
						}
					}
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(PlantEnt[i][X]))
				{
					...
					//Is Valid:
					if((StrContains(GetJob(i), "Drug Lord", false) != -1 || StrContains(GetJob(i), "Crime Lord", false) != -1 || GetDonator(i) > 0 || IsAdmin(i)) && !GetIsPlayerAfk(i))
					{
						...
						if(IsPlanted[i][X] == 1)
						{
							...
							if(PlantTime[i][X] > 1)
							{
								...
								if(Random == 1)
								{
									...
									//Initulize:
									float AddedGrams = GetRandomFloat(2.0, (4.0 + (float(PlantType[i][X]) / 1.5)) + (float(PlantType[i][X]) / 5));
...
public int CheckPlantToLamp(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Plant_Drug")) > 0)
	{

		//Check:
		if(IsInDistance(PlantEnt[Client][Y], Ent))
		{

			//Declare:
			int i = GetLampOwnerFromEnt(Ent);

			int Id = GetLampIdFromEnt(Ent);		
...
public void PlantHud(int Client, int Ent, float NoticeInterval)
{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(PlantEnt[i][X] == Ent)
				{
					...
					if(IsPlanted[i][X] == 0)
					{
						...
						//Format:
						len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Plant:\nHas no seed to grow anything!\nHealth: %i", PlantHealth[i][X]);
					}
					...
					else if(PlantTime[i][X] == 1)
					{
						...
						if(PlantOverGrownTime[i][X] == (MAXPLANTDIED / 2))
						{
							...
							//Format:
							len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Plant is dying!");
						}
					}
					...
					else
					{
						...
						if(StrContains(GetJob(i), "Drug Lord", false) != -1 || StrContains(GetJob(i), "Crime Lord", false) != -1 || GetDonator(i) > 0 || IsAdmin(i))
						{
							...
							if(PlantType[i][X] == 2)
							{
								...
								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "GMO Plant:\n");
							}
							...
							if(PlantType[i][X] == 3)
							{
								...
								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Haze Plant:\n");
							}
							...
							if(GetIsPlayerAfk(i))
							{
								...
								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Growing suspended\n");
							}
							...
							else
							{
								...
								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Finishes Growing In %i Sec\n", PlantTime[i][X]);
							}

							//Check:
							if(PlantWater[i][X] > 0.00)
							{

								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Plant Wanter: %.2f Percent\n", PlantWater[i][X]);
							}
							...
							else
							{
								...
								//Format:
								len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Plant Wanter: No Water\n", PlantWater[i][X]);
							}
...
public void RemovePlant(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(PlantEnt[Client][X]) && PlantEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, PlantEnt[Client][X]);
	}
...
public bool CreatePlant(int Client, int Id, int Time, float Value, float Water, int Planted, int Type, int Health, float Position[3], float Angle[3], bool IsConnected)
{
	...
	if(Planted >= 0)
	{
		...
		//Grow:
		if(PlantTime[Client][Id] <= 700 && PlantTime[Client][Id] > 600)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage2.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 600 && PlantTime[Client][Id] > 500)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage3.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 500 && PlantTime[Client][Id] > 400)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage4.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 400 && PlantTime[Client][Id] > 300)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage5.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 300 && PlantTime[Client][Id] > 200)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage6.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 200 && PlantTime[Client][Id] > 100)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage7.mdl");
		}

		//Grow:
		if(PlantTime[Client][Id] <= 100 && PlantTime[Client][Id] >= 1)
		{

			//Set Entity Model:
			SetEntityModel(PlantEnt[Client][Id],  "models/pot/pot_stage8.mdl");
		}
	}
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				//Spawn Plant:
				if(CreatePlant(Client, Y, 0, 0.0, 0.0, 0, 0, 500, Position, EyeAngles, false))
				{
