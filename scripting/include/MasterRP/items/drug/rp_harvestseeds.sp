...
public bool IsValidSeeds(int Ent)
{
...
public void SeedsHud(int Client, int Ent, float NoticeInterval)
{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(SeedsEnt[i][X] == Ent)
				{
					...
					if(SeedsType[i][X] == 3)
					{
						...
						Format(FormatMessage, sizeof(FormatMessage), "Haze Drug Seeds:\nHealth: %i", SeedsHealth[i][X]);
					}
...
public void RemoveSeeds(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(SeedsEnt[Client][X]) && SeedsEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, SeedsEnt[Client][X]);
	}
...
public Action Command_TestSeeds(int Client, int Args)
{
	...
	if(Args < 3)
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Usage: sm_testSeeds <Id> <1-2> <Health>");
