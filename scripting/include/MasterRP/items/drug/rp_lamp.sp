...
int LampType[MAXPLAYERS + 1][MAXITEMSPAWN + 1];
...
public int GetLampType(int Client, int Id)
{

	//Return:
	return LampType[Client][Id];
}
...
public void LampHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveLamp(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(LampEnt[Client][X]) && LampEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, LampEnt[Client][X]);
	}
...
public bool CreateLamp(int Client, int Id, int Type, int Health, float Position[3], float Angle[3], bool IsConnected)
{
	...
	if(IsConnected == false)
	{
		...
		//Format:
		Format(AddedData, sizeof(AddedData), "%i", Type);

		//Add Spawned Item to DB:
		InsertSpawnedItem(Client, 20, Id, 0, 0, Health, AddedData, Position, Angle);
