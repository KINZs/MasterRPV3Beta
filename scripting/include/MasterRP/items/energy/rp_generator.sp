...
public void GeneratorHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveGenerator(int Client, int X, bool Result)
{
	...
	//EntCheck:
	if(CheckMapEntityCount() < 2047 && Result && IsValidEdict(GeneratorEnt[Client][X]))
	{
	...
	//Check
	if(IsValidEdict(GeneratorEnt[Client][X]) && GeneratorEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, GeneratorEnt[Client][X]);
	}
