...
//GunLab:
int GunLabEnt[MAXPLAYERS + 1][MAXITEMSPAWN + 1];
int GunLabUse[MAXPLAYERS + 1][MAXITEMSPAWN + 1];
int GunLabTime[MAXPLAYERS + 1][MAXITEMSPAWN + 1];
char GunLabModel[256] = "models/props_c17/TrapPropeller_Engine.mdl";

public void initGunLab()
{
...
public void initDefaultGunLab(int Client)
{
...
public int HasClientGunLab(int Client, int Id)
{
...
public int GetGunLabTime(int Client, int Id)
{
...
public int GetGunLabUse(int Client, int Id)
{
...
public void OnGunLabUse(int Client, int Ent)
{
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(GunLabEnt[i][X]))
				{
					...
					//Check:
					if(!GetIsPlayerAfk(i)) CheckGeneratorToGunLab(i, X);
...
public void CheckGeneratorToGunLab(int Client, int Y)
{
	...
	while ((Ent = FindEntityByClassname(Ent, "prop_Generator")) > 0)
	{
		...
		//Check:
		if(IsInDistance(GunLabEnt[Client][Y], Ent))
		{
...
#if defined HL2DM
//This function needs rewriting to become compatable with other games
							//Spawn Weapon:
						...
						//Declare:
						int TempEnt = GetEntAttatchedEffect(GunLabEnt[Client][Y], 0);
...
public void GunLabHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveGunLab(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(GunLabEnt[Client][X]) && GunLabEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, GunLabEnt[Client][X]);
	}
...
public bool CreateGunLab(int Client, int Id, int Time, int Value, float Position[3], float Angle[3], bool IsConnected)
{
	...
	//Set Prop ClassName
	SetEntityClassName(Ent, "prop_Gun_Lab");
...
//Create Garbage Zone:
public Action Command_TestGunLab(int Client, int Args)
{
...
public void OnItemsGunLabUse(int Client, int ItemId)
{
