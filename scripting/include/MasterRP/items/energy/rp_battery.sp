		...
		if(IsClientConnected(i) && IsClientInGame(i) && !GetIsPlayerAfk(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(BatteryEnt[i][X]))
				{
					...
					//Check:
					if(!GetIsPlayerAfk(i)) CheckGeneratorToBattery(i, X);
...
public void CheckGeneratorToBattery(int Client, int Y)
{
	...
	while ((Ent = FindEntityByClassname(Ent, "prop_Generator")) > 0)
	{
		...
		//Check:
		if(IsInDistance(BatteryEnt[Client][Y], Ent))
		{
			...
			if(GetGeneratorEnergy(Client, Id) - 0.10 > 0)
			{
				...
				//Check:
				if(BatteryEnergy[Client][Y] < 2000)
				{
...
public void BatteryHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveBattery(int Client, int X, bool Result)
{
	...
	//Check
	if(IsValidEdict(BatteryEnt[Client][X]) && BatteryEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, BatteryEnt[Client][X]);
	}
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				//Declare:
				float fEnergy = StringToFloat(GetItemVar(ItemId));
