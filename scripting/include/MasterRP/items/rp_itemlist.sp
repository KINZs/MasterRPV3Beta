...
public void initItemList()
{
	...
	RegAdminCmd("sm_setitem", CommandSetItem, ADMFLAG_ROOT, "<Id 1-500> <Item> <Variable> - Sets an Item");
...
public Action CommandListItems(int Client, int Args)
{
	...
	if(Client == 0)
	{
		...
		//Print:
		PrintToServer("|RP| - Items (%i - %i):", 1, MAXITEMS);
		...
		for(int X = 0; X <= MAXITEMS; X++)
		{
			...
			//Not Created Tables:
			SQL_TQuery(GetGlobalSQL(), T_DBPrintItemList, query, 69);
		}
	...
	else 
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			if(ItemId > 0 && ItemId < MAXITEMS)
			{
				...
				{
					//Print:
					PrintToServer("%i - %s / %i / %i / %s Group %i", ItemId, Name, Cost, ActionId, Var, Group);
				}
			}
		}
	}
...
public Action CommandCreateItem(int Client, int Args)
{
	...
	if(Args < 6)
	{
		...
		if(Client == 0)
		{
			...
			//Print:
			PrintToServer("|RP| - Usage: sm_createitem <id> <name> <type> <variables> <cost> <Group>");
		}
	...
	if(Client == 0)
	{
		...
		//Print:
		PrintToServer("|RP| - Added Item %s - %s, Type: %s @ %s, Group %s", ItemId, Buffers[0], Buffers[1], Buffers[2], Buffers[4]);
	}
...
public Action CommandRemoveItem(int Client, int Args)
{
	...
	if(Args < 1)
	{
		...
		if(Client == 0)
		{
			...
			//Print:
			PrintToServer("|RP| - Usage: sm_removeitem <id>");
		}
	...
	if(Client == 0)
	{
		...
		//Print:
		PrintToServer("|RP| - Removed Item %s from the database", ItemId);
	}
...
//Edit Item:
public Action CommandSetItem(int Client, int Args)
{
	...
	if(Args != 3)
	{
		...
		//Print:
		CPrintToChat(Client, "%s - Usage: sm_setitem <Name> <ItemId> <Amount #>", PREFIX);
	...
	if(ItemId < 0 && ItemId > MAXITEMS)
	{
		...
		//Print:
		CPrintToChat(Client, "%s - Invalid ItemId <0-%i>", PREFIX, MAXITEMS);
	...
	//SetItem:
	SaveItem(Player, ItemId, Amount);

	//Print:
	CPrintToChat(Client, "%s you have set %N - %s - %i", PREFIX, Player, GetItemName(ItemId), Amount);
	if(Player != Client) CPrintToChat(Client, "%s %N  has set your - %s - %i", PREFIX, Client, GetItemName(ItemId), Amount);
