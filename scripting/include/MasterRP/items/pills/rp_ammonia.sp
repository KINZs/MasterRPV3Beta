...
public void AmmoniaHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveAmmonia(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(AmmoniaEnt[Client][X]) && AmmoniaEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, AmmoniaEnt[Client][X]);
	}
...
public bool CreateAmmonia(int Client,int Id, float fGrams, int Health, float Position[3], float Angle[3], bool IsConnected)
{
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				//Declare:
				float fGrams = StringToFloat(GetItemVar(ItemId));
