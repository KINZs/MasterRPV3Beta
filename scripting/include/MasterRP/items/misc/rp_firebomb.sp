...
public void FireBombHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveFireBomb(int Client, int X, bool Explode)
{
	...
	//Check
	if(IsValidEdict(FireBombEnt[Client][X]) && FireBombEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, FireBombEnt[Client][X]);
	}
