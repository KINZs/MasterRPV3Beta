...
public void FireExtinguisherHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveFireExtinguisher(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(FireExtinguisherEnt[Client][X]) && FireExtinguisherEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, FireExtinguisherEnt[Client][X]);
	}
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				if(CreateFireExtinguisher(Client, Y, StringToFloat(GetItemVar(ItemId)), Position, EyeAngles, false))
				{
