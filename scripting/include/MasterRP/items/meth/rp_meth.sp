					...
					for(int X = 1; X < MAXITEMSPAWN; X++)
					{
						...
						if(MethEnt[i][X] == Ent)
						{
							...
							else if(Grams > 0.0)
							{
								...
								//Initulize:
								SetMeth(Client, RoundFloat(float(GetMeth(Client)) + Grams));
							...
							if(Grams == 0.0)
							{
								...
								//Print:
								CPrintToChat(Client, "\x07FF4040|RP-Meth|\x07FFFFFF - You have collected no grams from your Kitchen!");
							}
							...
							if(Client == i)
							{
								...
								//Print:
								CPrintToChat(Client, "\x07FF4040|RP-Meth|\x07FFFFFF - You have collected â‚¬\x0732CD32%0.2f\x07FFFFFF from your Kitchen!", Grams);
							}
							...
							else
							{
								...
								//Save:
								SaveItem(Client, 242, (GetItemAmount(Client, 242) + 1));
								...
								//Print:
								CPrintToChat(i, "\x07FF4040|RP-Meth|\x07FFFFFF - %N has stolen your Meth Kitchen!", Client);

								CPrintToChat(Client, "\x07FF4040|RP-Meth|\x07FFFFFF - You have stolen a Meth Kitchen!");
							}
						}
					}
				}
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(MethEnt[i][X]))
				{
					...
					//Is Valid:
					if((StrContains(GetJob(i), "Meth Technician", false) != -1 || StrContains(GetJob(i), "Crime Lord", false) != -1 || GetDonator(i) > 0 || IsAdmin(i)) && !GetIsPlayerAfk(i))
					{
...
public void MethHud(int Client, int Ent, float NoticeInterval)
{
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(MethEnt[i][X] == Ent)
				{
					...
					//Format:
					len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Meth Kitchen:\n");

					//Has Grams:
					if(MethGrams[i][X] > 0.00)
					{
...
public void RemoveMeth(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(MethEnt[Client][X]) && MethEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, MethEnt[Client][X]);
	}
...
public int CheckPropaneTankDistanceToMethLab(int Client, int Y)
{
	...
	while ((Ent = FindEntityByClassname(Ent, "prop_Propane_Tank")) > 0)
	{
		...
		//Check:
		if(IsInDistance(MethEnt[Client][Y], Ent))
		{
...
public int CheckPhosphoruTankDistanceToMethLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Phosphoru_Tank")) > 0)
	{
...
public int CheckAcetoneCanDistanceToMethLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Acetone_Can")) > 0)
	{
...
public int CheckHcAcidTubDistanceToMethLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_HcAcid_Tub")) > 0)
	{
...
public int CheckSodiumTubDistanceToMethLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Sodium_Tub")) > 0)
	{
