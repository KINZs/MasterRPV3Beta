...
public void HcAcidTubHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveHcAcidTub(int Client,int  X)
{
	...
	//Check
	if(IsValidEdict(HcAcidTubEnt[Client][X]) && HcAcidTubEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, HcAcidTubEnt[Client][X]);
	}
