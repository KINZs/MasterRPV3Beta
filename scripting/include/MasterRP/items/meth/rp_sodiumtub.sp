...
public void SodiumTubHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveSodiumTub(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(SodiumTubEnt[Client][X]) && SodiumTubEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, SodiumTubEnt[Client][X]);
	}
...
public bool CreateSodiumTub(int Client, int Id, float fGrams, int Health, float Position[3], float Angle[3], bool IsConnected)
{
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				//Declare:
				float Grams = StringToFloat(GetItemVar(ItemId));
