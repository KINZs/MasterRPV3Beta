...
public void PhosphoruTankHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemovePhosphoruTank(int Client, int X, bool Result)
{
	...
	//EntCheck:
	if(CheckMapEntityCount() < 1900 && Result && IsValidEdict(PhosphoruTankEnt[Client][X]))
	{
	...
	//Check
	if(IsValidEdict(PhosphoruTankEnt[Client][X]) && PhosphoruTankEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, PhosphoruTankEnt[Client][X]);
	}
