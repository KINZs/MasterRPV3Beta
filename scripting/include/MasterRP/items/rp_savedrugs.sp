...
public void initSaveDrugs()
{
	...
	RegAdminCmd("sm_setmetal", Command_SetMetal, ADMFLAG_ROOT, "<Name> <Amount #> - Sets Resources");
}
...
public Action CreateSQLdbDynamicDrugs(Handle Timer)
{
	...
	len += Format(query[len], sizeof(query)-len, " `Metal` int(12) NOT NULL DEFAULT 0);");
...
public void T_LoadDrugsCallBack(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		else if(SQL_FetchRow(hndl))
		{
			...
			//Database Field Loading INTEGER:
			Metal[Client] = SQL_FetchInt(hndl, 8);
...
public int GetResources(int Client)
{
...
public int GetMetal(int Client)
{
...
public void SetMetal(int Client, int Amount)
{
	...
	if(IsLoaded(Client))
	{
		...
		//Sql Strings:
		Format(query, sizeof(query), "UPDATE Drugs SET Metal = %i WHERE STEAMID = %i;", Amount, SteamIdToInt(Client));
...
public Action Command_SetHarvest(int Client, int Args)
{
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have set \x0732CD32%N\x07FFFFFF Harvest to \x0732CD32%i", Client, iAmount);
...
public Action Command_SetMeth(int Client, int Args)
{
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have set \x0732CD32%N\x07FFFFFF Meth to \x0732CD32%i", Client, iAmount);
...
public Action Command_SetPills(int Client, int Args)
{
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have set \x0732CD32%N\x07FFFFFF Pills to \x0732CD32%i", Client, iAmount);
...
public Action Command_SetCocain(int Client, int Args)
{
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have set \x0732CD32%N\x07FFFFFF Cocain to \x0732CD32%i", Client, iAmount);
...
public Action Command_SetBitCoin(int Client, int Args)
{
	...
	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Set \x0732CD32%N\x07FFFFFF's BitCoin to \x0732CD32%f", Player, iAmount);
...
public Action Command_SetMetal(int Client, int Args)
{
	...
	if(Args != 2)
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Usage: sm_setmetal <Name> <Amount #>");
	...
	//Action:
	SetMetal(Player, iAmount);

	//Is Valid:
	if(Crime[Client] > 500) SetClientScore(Client, RoundToNearest(Crime[Client] / 1000.0));

	//Print:
	CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have set \x0732CD32%N\x07FFFFFF Metal to \x0732CD32%i", Client, iAmount);

	//Not Client:
	if(Client != Player) CPrintToChat(Player, "\x07FF4040|RP|\x07FFFFFF - \x0732CD32%N\x07FFFFFF set your Metal to \x0732CD32%i", Client, iAmount);
#if defined DEBUG
	//Logging:
	LogMessage("\"%L\" set the Metal of \"%L\" to %i", Client, Player, iAmount);
#endif
	//Return:
	return Plugin_Handled;
}
