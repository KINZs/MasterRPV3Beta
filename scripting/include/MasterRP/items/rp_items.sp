...
//Define:
#define MAXITEMSGROUPS		20
#define MAXDROPPEDITEMPROPS	50
...
float GpsBugTime[MAXPLAYERS + 1] = {0.0,...};
int GpsBug[MAXPLAYERS + 1] = {0,...};
...
public void initItems()
{
	...
	RegConsoleCmd("sm_inv", Command_Inv);

	//Command Listener:
	AddCommandListener(DisableMenuBind, "sm_vmenuselect");
...
public void DefaultItems(int Client)
{
	...
	GpsBugTime[Client] = GetGameTime();

	GpsBug[Client] = -1;

	SetHackItemDefaults(Client);
}
...
public void AddItemSteamId(int SteamId, int ItemId, int Amount)
{
	...
	for(int i = 1; i <= GetMaxClients(); i ++)
	{
		...
		//Format:
		Format(query, sizeof(query), "SELECT * FROM `Items` WHERE STEAMID = %i AND ItemId = %i;", SteamId, ItemId);
		...
		if(hQuery)
		{
			...
			if(Fetch)
			{
				...
				//Format:
				Format(query, sizeof(query), "UPDATE Items SET Amount = %i WHERE STEAMID = %i AND ItemId = %i;", (Amount + SQL_FetchInt(hQuery, 2)), SteamId, ItemId);
			}
			...
			else
			{
				...
				//Format:
				Format(query, sizeof(query), "INSERT INTO Items (`STEAMID`,`ItemId`,`Amount`) VALUES (%i,%i,%i);", SteamId, ItemId, Amount);
			}

			//Not Created Tables:
			SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query, 1260);
		}
	...
	if(!IsCuffed(Client))
	{
		...
		if(!GetIsCritical(Client))
		{
			...
			if(IsClientInTrading(Client))
			{
				...
				//Menu Button:
				menu.AddItem("3", "Market Place!");
			}

			//Menu Button:
			menu.AddItem("4", "Settings Menu");
...
public Action Command_JailPass(int Client, int Args)
{
	...
	if(!IsCuffed(Client))
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You can only use a 'Jail Free Pass' while in jail!");
	...
	for(int X = 0; X < MAXITEMS; X++)
	{
		...
		if(ItemAction[X] == 41)
		{
			...
			if(Item[Client][X] > 0)
			{
				...
				//Save:
				SaveItem(Client, X, (Item[Client][X] - 1));
...
public Action Inventory(int Client)
{
	...
	if(IsClientInTrading(Client))
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You are in a trading zone, you can't use your inventory in a trading zone!");
	...
	for(int X = 0; X < MAXITEMS; X++)
	{
		...
		if(Item[Client][X] > 0)
		{
			...
			//Trail Items:
			if(ItemGroup[X] == 16 && MenuShow[16] != 1)
			{
			...
			//Trail Items:
			if(ItemGroup[X] == 17 && MenuShow[17] != 1)
			{
			...
			//Trail Items:
			if(ItemGroup[X] == 18 && MenuShow[18] != 1)
			{
...
public int HandlePrompt(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		//Connected:
		else if(IsPlayerAlive(Client) && !IsCuffed(Client) && !GetIsCritical(Client))
		{
		...
		{
			//Print:
			CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You are cuffed, you are unable to use your inventory!");
		}
	}
...
public void DropItemMenu(int Client)
{
	...
	//Format:
	Format(title, sizeof(title), "How many items would you\nlike to drop?\n\nItem: %s", ItemName[SelectedItem[Client]]);

	//Menu Title:
	menu.SetTitle(title);

	//Format:
	Format(display, sizeof(display), "All (%i)", Item[Client][SelectedItem[Client]]);

	//Convert:
	IntToString(Item[Client][SelectedItem[Client]], info, sizeof(info));
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		else
		{
			...
			if(Item[Client][SelectedItem[Client]] - Amount >= 0 && Amount > 0)
			{
				...
				//Is Disabled:
				if(lastspawn[Client] > (GetGameTime() - 2))
				{
				...
				else
				{
					...
					//Check:
					if(GetItemPropsOnMap() >= MAXDROPPEDITEMPROPS)
					{

						//Can Replace Prop:
						if(ReplaceItemProp() == false)
						{

							//Print:
							CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You can't drop anymore items because there are too many item props!");
...
public Action UseItem(int Client, int ItemId, int Amount)
{
	...
	//Is In Time::
	if(lastspawn[Client] > (GetGameTime() - 0.2))
	{

		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Itembinding is not allowed here!");
	...
	if(InVehicle > -1)
	{
		...
		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - you can't use your inventory while in a vehicle!");
				...
				if(Item[Client][ItemId] - Amount >= 0)
				{
					...
					switch(ItemAction[ItemId])
					{
						...
						case 1:
						{
							...
							//Initulize:
							OnItemsWeaponUse(Client, ItemId, Amount, true);
						...
						case 5:
						{
							...
							//LockPick:
							OnItemsUseLockPick(Client, ItemId, Var);
						...
						case 6:
						{
							...
							//DoorHack:
							OnItemsUseDoorHack(Client, ItemId, Var);
						...
						case 9:
						{
							...
							//CuffSawe:
							OnClientUseItemCuffSaw(Client, ItemId, Var);
						...
						case 11:
						{
							...
							//Forward to rp_doorlocked.sp
							OnItemsUseAddLocks(Client, ItemId);
						...
						case 12:
						{
							...
							//Forward to rp_doorlocked.sp
							OnItemsUseRemLocks(Client, ItemId);
						...
						case 36:
						{
							...
							if(PoliceJammerTime[Client] > GetGameTime())
							{
								...
								//Print:
								CPrintToChat(Client, "%s You have already useda Police Jammer.", PREFIX);
							}
							...
							else
							{
								...
								//Print:
								CPrintToChat(Client, "%s Initializing Police Jammer for %s%i%s Seconds.", PREFIX, COLORGREEN, (60*Var), COLORWHITE);
						...
						case 37:
						{
							...
							if(PoliceScannerTime[Client] > GetGameTime())
							{
								...
								//Print:
								CPrintToChat(Client, "%s You have already useda Police Scanner.", PREFIX);
							}
							...
							else
							{
								...
								//Print:
								CPrintToChat(Client, "%s Initializing Police Detector for %s%i%s Seconds.", PREFIX, COLORGREEN, (60*Var), COLORWHITE);
						...
						case 38:
						{
							...
							if(BountyJammerTime[Client] > GetGameTime())
							{
								...
								//Print:
								CPrintToChat(Client, "%s You have already used a Bounty Jammer.", PREFIX);
							}
							...
							else
							{
								...
								//Print:
								CPrintToChat(Client, "%s Initializing Bounty Jammer for %s%i%s Seconds.", PREFIX, COLORGREEN, (60*Var), COLORWHITE);
						...
						case 60:
						{
							...
							//Set Jetpack Effect
							SetJetPackEffect(Client, Var);

							//Print:
							CPrintToChat(Client, "%s Your new JetPack Effect is a %s%s!", PREFIX, COLORGREEN, ItemName[ItemId]);
						...
						case 62:
						{
							...
							//Handle Ammonia:
							OnItemsAmmoniaUse(Client, ItemId);
						...
						case 63:
						{
							...
							//Handle SAcid Tub:
							OnItemsSAcidTubUse(Client, ItemId);
						...
						case 64:
						{
							...
							//Handle Toulene:
							OnItemsTouleneUse(Client, ItemId);
						...
						case 65:
						{
							...
							OnFireExtinguisherUse(Client, ItemId);
						...
						case 69:
						{
							...
							//Print:
							CPrintToChat(Client, "%s you can only spawn your vehicle at the cars npc!", PREFIX);
						...
						case 70:
						{
							...
							if(Var == 1)
							{
								...
								//Forward:
								OnItemsUseHackComputer(Client, ItemId);
							}
							...
							if(Var == 2)
							{
								...
								//Forward:
								OnItemsUseBankHackSoftware(Client, ItemId);
							}
							...
							if(Var == 3)
							{
								...
								//Forward:
								OnItemsUseVendorSoftware(Client, ItemId);
							}
							...
							if(Var == 4)
							{
								...
								//Forward:
								OnItemsUseNoCrimeSoftware(Client, ItemId);
							}
							...
							if(Var == 5)
							{
								...
								//Forward:
								OnItemsUseHackingSoftware(Client, ItemId);
							}
							...
							if(Var == 6)
							{
								...
								//Forward:
								OnItemsUseBankSoftware(Client, ItemId);
							}
						}
						...
						case 72:
						{
							...
							//DoorHack:
							OnItemsUseMultiHack(Client, ItemId, Var);
						...
						case 73:
						{
							...
							//Already has:
							if(GpsBugTime[Client] > GetGameTime())
							{
							...
							else
							{
								...
								//Is Valid:
								if(Ent > 1 && Ent <= GetMaxClients() && IsClientConnected(Ent))
								{
									...
									//Print:
									CPrintToChat(Client, "%s Initializing GPS Bug on %s%N%s for %s%i%s Seconds!", PREFIX, COLORGREEN, COLORWHITE, Ent, COLORGREEN, (60*Var), COLORWHITE);

									//Initulize:
									GpsBugTime[Client] = GetGameTime() + (60*Var);
								...
								else
								{
									...
									//Print:
									CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You need to point at a player to use this item.");
								}
							}
...
public int FindClientItemFromItemAction(int Client, int Result)
{
...
public void SetItemVar(int ItemId, const char[] Str)
{
...
public float GetGpsBugTime(int Client)
{

	//Return:
	return GpsBugTime[Client];
}

public int GetGpsBug(int Client)
{

	//Return:
	return GpsBug[Client];
}

public int SetGpsBug(int Client, int Player)
{

	//Initulize:
	GpsBug[Client] = Player;
}

public float GetDoorHackTime(int Client)
{

	//Return:
	return view_as<float>(HackTime[Client]);
}

public void SetDoorHackTime(int Client, float Time)
{

	//Initulize:
	HackTime[Client] = Time;
}
...
public int GetMaxItems()
{
	...
	//Return:
	return view_as<int>(MAXITEMS);
}

public int GetItemPropsOnMap()
{
	...
	//Switch:
	while ((Props = FindEntityByClassname(Props, "prop_Dropped_Item")) != -1)
	{
...
public void RemoveItemProps()
{

	//Loop:

	for(int Ent = GetMaxClients() + 1; Ent < 2047; Ent++)
 for(int ItemId = 0; ItemId < MAXITEMS; ItemId++)
	{

		//Check:
		if(ItemAmount[Ent][ItemId] > 0)
		{
			...
			//Initialize:
			ItemAmount[Ent][ItemId] = 0;
		}
	}
}

public bool ReplaceItemProp()
{
...
public float GetLastSpawn(int Client)
{

	//Return:
	return view_as<float>(lastspawn[Client]);
}

public void SetLastSpawn(int Client, float Result)
{

	//Initulize:
	lastspawn[Client] = Result;
}

public Action DisableMenuBind(int Client, const char[] Command, int Argc)
{
	...
	//Is In Time:
	if(lastspawn[Client] > (GetGameTime() - 0.1))
	{

		//Print:
		PrintToConsole(Client, "|RP| - Menubinding is not allowed here!");
