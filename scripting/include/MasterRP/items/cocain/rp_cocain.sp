					...
					for(int X = 1; X < MAXITEMSPAWN; X++)
					{
						...
						if(CocainEnt[i][X] == Ent)
						{
							...
							else if(Grams > 0.0)
							{
								...
								//Initulize:
								SetCocain(Client, RoundFloat(float(GetCocain(Client)) + Grams));
							...
							if(Grams == 0.0)
							{
								...
								//Print:
								CPrintToChat(Client, "\x07FF4040|RP-Cocain|\x07FFFFFF - You have collected no grams from your Kitchen!");
							}
							...
							if(Client == i)
							{
								...
								//Print:
								CPrintToChat(Client, "\x07FF4040|RP-Cocain|\x07FFFFFF - You have collected â‚¬\x0732CD32%0.2f\x07FFFFFF from your Kitchen!", Grams);
							}
							...
							else
							{
								...
								//Save:
								SaveItem(Client, 246, (GetItemAmount(Client, 246) + 1));
								...
								//Print:
								CPrintToChat(i, "\x07FF4040|RP-Cocain|\x07FFFFFF - %N has stolen your Cocain Kitchen!", Client);

								CPrintToChat(Client, "\x07FF4040|RP-Cocain|\x07FFFFFF - You have stolen a Cocain Kitchen!");
							}
						}
					}
				}
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(CocainEnt[i][X]))
				{
					...
					//Is Valid:
					if((StrContains(GetJob(i), "Cocain Technician", false) != -1 || StrContains(GetJob(i), "Crime Lord", false) != -1 || GetDonator(i) > 0 || IsAdmin(i)) && !GetIsPlayerAfk(i))
					{
...
public void CocainHud(int Client, int Ent, float NoticeInterval)
{
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(CocainEnt[i][X] == Ent)
				{
					...
					//Format:
					len += Format(FormatMessage[len], sizeof(FormatMessage)-len, "Cocain Kitchen:\n");

					//Has Grams:
					if(CocainGrams[i][X] > 0.00)
					{
...
public void RemoveCocain(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(CocainEnt[Client][X]) && CocainEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, CocainEnt[Client][X]);
	}
...
public int CheckErythroxylumDistanceToCocainLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Erythroxylum")) > 0)
	{

		//Check:
		if(IsInDistance(CocainEnt[Client][Y], Ent))
		{
...
public int CheckBenzocaineDistanceToCocainLab(int Client, int Y)
{
	...
	//Switch:
	while ((Ent = FindEntityByClassname(Ent, "prop_Benzocaine")) > 0)
	{
