...
public void initProps()
{
	...
	RegAdminCmd("sm_color", Command_SaveColor, ADMFLAG_SLAY, "- Changes the Angles on an object");

	RegAdminCmd("sm_render", Command_Render, ADMFLAG_SLAY, "- Changes the Render on an object");

	RegAdminCmd("sm_setsequance", Command_SetSequance, ADMFLAG_SLAY, "");
	...
	else 
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			if(IsValidEdict(X))
			{
				...
				//Request:
				RequestFrame(OnNextFrameKill, X);
		...
		//Print:
		PrintToServer("|RP| - %i Removed Map Props!", Props);
	}
}
...
public Action Command_RestoreMapProp(int Client, int Args)
{
	...
	//Format:
	Format(query, sizeof(query), "SELECT * FROM `RemovedProps` WHERE Map = '%s' AND PropId = %i;", ServerMap(), StringToInt(SpawnId));
...
public Action Command_SaveColor(int Client, int Args)
{
	
	if(Args < 3)
	{
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Usage: sm_color <red> <green> <blue> <alpha>");
		return Plugin_Handled;
	}
	...
	if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
	{
		...
		CPrintToChat(Client, "\x07FFFFFF\x07FF4040|RP|\x07FFFFFF - You set: red=\x0732CD32%d\x07FFFFFF g=\x0732CD32%d\x07FFFFFF b=\x0732CD32%d\x07FFFFFF a=\x0732CD32%d\x07FFFFFF",StringToInt(arg1), StringToInt(arg2), StringToInt(arg3), StringToInt(arg4));
	}
...
public Action Command_Render(int Client, int Args)
{
	...
	if(Args != 2)
	{
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - Usage: sm_render ");
		return Plugin_Handled;
	}
	...
	if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
	{
		...
		//Set Render:
		SetEntityRenderMode(Ent, IntToRenderMode(StringToInt(arg1)));

		//Set Render Ex:
		SetEntityRenderFx(Ent, IntToRenderModeFx(StringToInt(arg2)));

		CPrintToChat(Client, "\x07FFFFFF\x07FF4040|RP|\x07FFFFFF - You set the rendor: %i, renderfx: %i",StringToInt(arg1), StringToInt(arg2));
	}
...
public Action Command_SetSequance(int Client, int Args)
{
	...
	if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
	{
		...
		int Sequence = GetEntProp(Ent, Prop_Send, "m_nSequence");
		...
		if(Args == 1)
		{
			...
			//Declare:
			SetEntProp(Ent, Prop_Send, "m_nSequence", StringToInt(arg1));
		}

		//Print:
		CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - old sequance %i", Sequence);
	}
}