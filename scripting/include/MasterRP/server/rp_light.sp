...
public bool RemoveLight(int Ent)
{
	...
	if(IsValidEdict(LightEnt[Ent]))
	{
		...
		//Request:
		RequestFrame(OnNextFrameKill, LightEnt[Ent]);
...
public int IsLightOn(int Ent)
{

	//Return:
	return LightIsOn[Ent];
}
public void SetLightState(int Ent, int IsOn)
{
	...
	if(IsOn == 1)
	{
		...
		//Accept:
		AcceptEntityInput(Ent, "TurnOn", Ent);
	}
	...
	else
	{
		...
		//Accept:
		AcceptEntityInput(Ent, "TurnOff", Ent);
	}
...
public int GetLightEnt(int Ent)
{
...
public int SetLightEnt(int Ent, int Light)
{
