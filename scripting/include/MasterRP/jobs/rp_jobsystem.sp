...
public void initJobSytem()
{
	...
	RegConsoleCmd("sm_switch", Command_SwitchCop);
...
public Action CreateSQLdbDynamicJobs(Handle Timer)
{
	...
	len += Format(query[len], sizeof(query)-len, " `Energy` int(12) NOT NULL DEFAULT 100,");
...
public void T_LoadJobsCallBack(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	else 
	{
		...
		else if(SQL_FetchRow(hndl))
		{
			...
			//Database Field Loading INTEGER:
			Energy[Client] = SQL_FetchInt(hndl, 4);

			//Database Field Loading FLOAT:
			Hunger[Client] = SQL_FetchFloat(hndl, 5);
...
public void SetDefaultJob(int Client)
{
...
public void initSalaryTimer()
{
	...
	if(SalaryCheck == 1 || SalaryCheck == 60)
	{
		...
		for(int Client = 1; Client <= GetMaxClients(); Client++)
		{
			...
			if(IsClientConnected(Client) && IsClientInGame(Client))
			{
				...
				//Initulize:
				SetOnlineTime(Client, (GetOnlineTime(Client) + 1));

				//Sql Strings:
				len += Format(query[len], sizeof(query)-len, "UPDATE Player SET OnlineTime = %i WHERE STEAMID = %i;", GetOnlineTime(Client), SteamIdToInt(Client));
				...
				// Dynamic Economy!
				TakeServerSafeMoneyAll(GetJobSalary(Client));
	...
	for(int Client = 1; Client <= GetMaxClients(); Client++)
	{
		...
		if(IsClientConnected(Client) && IsClientInGame(Client))
		{
			...
			//Cosino:
			CheckClientIsInCosino(Client);

			//Trading
			CheckClientIsInTrading(Client);
		}
	}
...
public Action Command_SetHunger(int Client, int Args)
{
...
public Action Command_SetEnergy(int Client, int Args)
{
...
public Action Command_SetNextJobRase(int Client, int Args)
{
...
public Action Command_SetCopExperience(int Client, int Args)
{
...
public Action Command_SetCopCuffs(int Client, int Args)
{
...
public Action Command_SetCopMinutes(int Client, int Args)
{
...
public Action Command_SwitchCop(int Client, int Args)
{
	...
	//Is Cop:
	if(IsCop(Client) || StrContains(GetJob(Client), "Admin", false) != -1)
	{
		...
		if(GetCrime(Client) <= 500)
		{
			...
			if(!IsCuffed(Client))
			{
				...
				//Print:
				CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have switch to citizen.");
			...
			else
			{
				...
				//Print:
				CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You can't switch whilst you are cuffed");
			}
		}
		...
		else
		{
			...
			//Print:
			CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have too much crime too switch");
		}
		...
		if(StrContains(GetOrgJob(Client), "Police", false) != -1 || StrContains(GetOrgJob(Client), "SWAT", false) != -1 || StrContains(GetOrgJob(Client), "Admin", false) != -1)
		{
			...
			if(GetCrime(Client) <= 500)
			{
				...
				if(!IsCuffed(Client))
				{
					...
					//Print:
					CPrintToChat(Client, "\x07FF4040|RP|\x07FFFFFF - You have switched to cival protection.");
...
public void SetJob(int Client, const char[] Str)
{
	...
	//Load Job Experience for current job selected:
	LoadExperience(Client);
}
...
char GetHungerString(int Client)
{
...
public void SetCopCuffs(int Client, int Amount)
{
	...
	if(IsLoaded(Client))
	{
		...
		//Sql Strings:
		Format(query, sizeof(query), "UPDATE jobs SET CopCuffs = %i WHERE STEAMID = %i;", CopCuffs[Client], SteamIdToInt(Client));
