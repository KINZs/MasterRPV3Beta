...
public void WaterBombHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveWaterBomb(int Client, int X, bool Explode)
{
	...
	//Check
	if(IsValidEdict(WaterBombEnt[Client][X]) && WaterBombEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, WaterBombEnt[Client][X]);
	}
