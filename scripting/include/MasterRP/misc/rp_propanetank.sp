...
public void PropaneTankHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemovePropaneTank(int Client, int X, bool Result)
{
	...
	//EntCheck:
	if(CheckMapEntityCount() < 2047 && Result && IsValidEdict(PropaneTankEnt[Client][X]))
	{
	...
	//Check:
	if(IsValidEdict(PropaneTankEnt[Client][X]) && PropaneTankEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, PropaneTankEnt[Client][X]);
	}
	...
	else
	{
		...
		for(int Y = 1; Y <= MaxSlots; Y++)
		{
			...
			if(!IsValidEdict(Ent))
			{
				...
				//Check:
				if(Fuel > 1000) Level = 3;
				if(Fuel > 500) Level = 2;

				//CreatePropaneTank
				if(CreatePropaneTank(Client, Y, Fuel, Level, 500, Position, EyeAngles, false))
				{
