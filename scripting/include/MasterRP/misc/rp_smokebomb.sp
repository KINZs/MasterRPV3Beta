...
public void SmokeBombHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveSmokeBomb(int Client, int X, bool Explode)
{
	...
	//Check
	if(IsValidEdict(SmokeBombEnt[Client][X]) && SmokeBombEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, SmokeBombEnt[Client][X]);
	}
