...
public int GetRiceValue(int Client, int Id)
{
...
public void RiceHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveRice(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(RiceEnt[Client][X]) && RiceEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, RiceEnt[Client][X]);
	}
