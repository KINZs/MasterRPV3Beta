		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			...
			for(int X = 1; X < MAXITEMSPAWN; X++)
			{
				...
				if(IsValidEdict(MicrowaveEnt[i][X]))
				{
					...
					//Is Valid:
					if((StrContains(GetJob(i), "Microwave Technician", false) != -1 || StrContains(GetJob(i), "Crime Lord", false) != -1 || GetDonator(i) > 0 || IsAdmin(i)) && !GetIsPlayerAfk(i))
					{
...
public void MicrowaveHud(int Client, int Ent, float NoticeInterval)
{
...
public void RemoveMicrowave(int Client, int X)
{
	...
	//Check
	if(IsValidEdict(MicrowaveEnt[Client][X]) && MicrowaveEnt[Client][X] > GetMaxClients())
	{

		//Request:
		RequestFrame(OnNextFrameKill, MicrowaveEnt[Client][X]);
	}
