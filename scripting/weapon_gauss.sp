...
#define PLAYER_HIT_FORCE_CHARGE 5.0
#define CHARGELOOP_PITCH_START 50
#define CHARGELOOP_PITCH_END 250
#define GAUSS_CHARGE_TIME 0.3 // was 0.2
#define MAX_GAUSS_CHARGE_TIME 3.0 //10 ivervals by default
#define DANGER_GAUSS_CHARGE_TIME 10.0
#define GAUSS_CHARGE_DAMAGE_PERBULLET 20.0 // Changed Shot Multiply by 
#define GAUSS_DAMAGE_PERBULLET 30.0 //Single Shot
...
public Plugin myinfo =
{
	...
	description = "CustomGuns Weapon_Gauss Extension remade by Master(D) to be more like the original code",
	version = "00.00.01",
	url = ""
};
			...
			else
			{
				...
				if(Ammo != 0)
				{
					...
					if(flChargeAmount <= 1.0 )
					{
						...
						//Print:
						//PrintToServer("Test pitch = %i weapon = %i", newPitch, Weapon);
					}
					...
					if(m_flChargeTime[Client] + (DANGER_GAUSS_CHARGE_TIME - MAX_GAUSS_CHARGE_TIME) < GetGameTime())
					{
						...
						CG_GetShootPosition(Client, pos, 12.0, 8.0, -7.0);

						TE_SetupGaussExplosion(pos, 0, ang);
						TE_SendToAll();

						//Temp Ent:
						TE_SetupEnergySplash(pos, ang, true);

						//Show To Client:
						TE_SendToAll();

						//TE Setup:
						TE_SetupDynamicLight(pos, 255, 100, 10, 8, 25.0, 0.2, 50.0);
...
void PrimaryFire(int client, int weapon)
{
	...
	//TE Setup: 
	TE_SetupDynamicLight(startPos, 255, 100, 10, 8, 35.0, 0.2, 50.0);
	...
	else if(entityHit != -1)
	{
		if(IsPlayer(entityHit))
		{
			if(!teamplay || GetClientTeam(entityHit) != GetClientTeam(client))
			{
				...
				ScaleVector(dmgForce, PLAYER_HIT_FORCE_CHARGE);
				SDKHooks_TakeDamage(entityHit, client, client, GAUSS_DAMAGE_PERBULLET, DMG_SHOCK, weapon, dmgForce, endPos);
			}
		}
...
void ChargedFire(int client, int weapon, int Amount)
{
	...
	//Declare:
	float Dist2 = (10.0 + float(Amount * 5));

	//TE Setup: 
	TE_SetupDynamicLight(startPos, 255, 100, 10, 8, Dist2, 0.2, 50.0);
	...
	if(IsPlayer(entityHit))
	{
		...
		{
			float dmgForce[3];
			NormalizeVector(angles, dmgForce);
			ScaleVector(dmgForce, (Amount * PLAYER_HIT_FORCE_CHARGE));
			SDKHooks_TakeDamage(entityHit, client, client, (Amount * GAUSS_CHARGE_DAMAGE_PERBULLET), DMG_SHOCK, weapon, dmgForce, endPos);
		}
	}
	else
	{
	...
	//Declare:
	float Dist = (25.0 + float(Amount * 5));

	//TE Setup: 
	TE_SetupDynamicLight(endPos, 255, 100, 10, 8, Dist, 0.2, 50.0);
	...
	{
			//Temp Ent:
			TE_SetupSparks(endPos, traceNormal, 5, 5);
